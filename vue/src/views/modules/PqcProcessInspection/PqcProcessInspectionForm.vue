...
// 初始化表单数据
onMounted(() => {
  const formData = JSON.parse(sessionStorage.getItem('pqc-formData'));
  if (formData) {
    data.form = formData;
    // 读取后清除数据
    sessionStorage.removeItem('pqc-formData');
  }
});
...
// 新增数据
const add = () => { // 新增对象没有id
  request.post('pqcProcessInspection/add', data.form).then(res => {
    if (res.code === '200') {
...
const update = () => { // 编辑对象有id
  request.put('pqcProcessInspection/update', data.form).then(res => {
    if (res.code === '200') {
      data.formVisible = false
      ElMessage.success('操作成功')
    } else {
