...
<!--        <el-input v-model="data.id" placeholder="请输入检验编号查询" style="width: 240px; margin-right: 10px" prefix-icon="Search"></el-input>-->
        ...
        <el-input v-model="data.material" placeholder="请输入物料查询" style="width: 240px; margin-right: 10px" prefix-icon="Search" clearable></el-input>
    ...
    <div style="margin-bottom: 5px">
      <el-card>
        <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column label="物料名称" prop="material" />
...
const load = () => {
  ...
  request.get('pqcProcessInspection/selectPage', {
    params: {
...
const handleAdd = () => {
  ...
  router.push({ name: 'PqcProcessInspectionForm' });
};
...
const handleUpdate = (row) => {
  ...
  sessionStorage.setItem('currentPage', data.pageNum);
  // 使用本地存储临时缓存
  sessionStorage.setItem('pqc-formData', JSON.stringify(row))
  router.push({ name: 'PqcProcessInspectionForm' });
};
...
const del = (id) => {
  ElMessageBox.confirm('删除数据后无法恢复，您确认删除吗', "删除确认", { type: 'warning' }).then(() => {
    request.delete('pqcProcessInspection/deleteById/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除数据后无法恢复，您确认删除吗', "删除确认", { type: 'warning' }).then(() => {
    request.delete('pqcProcessInspection/deleteBatch', { data: data.ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功');
        load();
      } else {
