<template>
  ...
  <div>
    ...
    <div style="display: flex; grid-gap: 10px">
      <!-- 检验数量、不良率趋势图 -->
      <el-card style="flex: 1; height: 300px" id="pqcLine"></el-card>
      <!-- 物料合格占比饼图 -->
      <el-card style="flex: 1; height: 300px" id="pqcPie"></el-card>
    </div>
  </div>
</template>
...
const loadChartsData = () => {
  ...
  // 获取各生产车间的不良数量对比数据
  request.get('pqcProcessInspection/workshopDefectiveCount', {
  ...
  // 获取不同物料的不良数量对比数据
  request.get('pqcProcessInspection/materialDefectiveCount', {
    params: { startDate, endDate }
  }).then(res => {
    if (res.code === '200') {
      const data = res.data;
  ...
  // 获取检验数量、不良率趋势数据
  request.get('pqcProcessInspection/trendData', {
    params: { startDate, endDate }
  }).then(res => {
    if (res.code === '200') {
      const data = res.data;
      ...
      pqcLineOption.xAxis.data = dates;
      pqcLineOption.series[0].data = incomingQuantities;
      pqcLineOption.series[1].data = defectiveRates;
      chartInstances.pqcLine.setOption(pqcLineOption);
    } else {
      ElMessage.error(res.msg)
    }
  });
  // 获取检验结果数据
  request.get('pqcProcessInspection/inspectionResultRatio', {
    params: { startDate, endDate }
  }).then(res => {
    if (res.code === '200') {
      const data = res.data;
      ...
      pqcPieOption.series[0].data[0].value = qualifiedData.resultCount;
      pqcPieOption.series[0].data[1].value = unqualifiedData.resultCount;
      chartInstances.pqcPie.setOption(pqcPieOption);
    } else {
      ElMessage.error(res.msg)
    }
  });
